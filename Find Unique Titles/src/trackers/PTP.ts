...
const canUploadTorrent = (torrent: Torrent, availableTorrents: Array<Torrent>) => {
    const similarTorrents = availableTorrents.filter(e => {
        return (torrent.resolution === "SD" || (e.resolution === torrent.resolution)) &&
            (torrent.container === undefined || sameContainer(e.container, torrent.container)) &&
